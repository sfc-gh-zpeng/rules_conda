...
def _update_environment(rctx, executable, env_name, env_file):
    ...
    if rctx.attr.coverage:
      rctx.report_progress("Installing coverage: {}".format(rctx.attr.coverage))
      result = rctx.execute([executable, "install", "-y", "--freeze-installed", "-p", "./{}".format(env_name), rctx.attr.coverage])
      if result.return_code:
        fail("Failure installing coverage ({}).\nstdout: {}\nstderr: {}".format(rctx.attr.coverage, result.stdout, result.stderr))
...
conda_create_rule = repository_rule(
    ...
    attrs = {
        ...
        "coverage": attr.string(
            default = "",
            doc = "conda version spec of the coverage tool (e.g. \"coverage==6.3.0\")",
        ),
    },
)
